...
namespace thienthu1410\BehatHTMLFormatter;
...
class BehatHTMLFormatterExtension implements ExtensionInterface {
  ...
  public function configure(ArrayNodeDefinition $builder) {
    $builder->children()->scalarNode("name")->defaultValue("thienthu1410html");
    ...
    $notCountTag = [];
    $builder->children()->scalarNode('not_count_tag')->defaultValue($notCountTag);
  }
  ...
  public function load(ContainerBuilder $container, array $config) {
    $definition = new Definition("thienthu1410\\BehatHTMLFormatter\\Formatter\\BehatHTMLFormatter");
    ...
    if (!empty($config['not_count_tag']))
        $definition->addArgument(explode(",", $config['not_count_tag']));
    else
        $definition->addArgument($config['not_count_tag']);
